#!/usr/bin/env python3
"""
Compilation script for the assembly debug test
"""
...
def compile_assembly_debug_test():
    """Compile the assembly debug test"""
    logging.basicConfig(level=logging.DEBUG)
    
    current_dir = os.path.dirname(os.path.abspath(__file__))
    stub_source = os.path.join(current_dir, "assembly_debug_test.c")
    stub_obj = os.path.join(current_dir, "assembly_debug_test.o")
    stub_elf = os.path.join(current_dir, "assembly_debug_test.elf")
    stub_blob = os.path.join(current_dir, "assembly_debug_test_compiled.bin")
    ...
    try:
        ...
        logging.info(f"Assembly debug test compiled successfully to blob: {stub_blob}")
        print(f"Assembly debug test blob created at: {stub_blob}")
