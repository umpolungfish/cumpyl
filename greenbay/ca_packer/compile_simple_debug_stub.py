#!/usr/bin/env python3
"""
Compilation script for the simple debug stub
"""
...
def compile_simple_debug_stub():
    """Compile the simple debug stub"""
    logging.basicConfig(level=logging.DEBUG)
    
    current_dir = os.path.dirname(os.path.abspath(__file__))
    stub_source = os.path.join(current_dir, "simple_debug_stub.c")
    stub_obj = os.path.join(current_dir, "simple_debug_stub.o")
    stub_elf = os.path.join(current_dir, "simple_debug_stub.elf")
    stub_blob = os.path.join(current_dir, "simple_debug_stub_compiled.bin")
    ...
    try:
        ...
        logging.info(f"Simple debug stub compiled successfully to blob: {stub_blob}")
        print(f"Simple debug stub blob created at: {stub_blob}")
