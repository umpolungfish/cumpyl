#!/usr/bin/env python3
"""
Compilation script for the minimal parameter stub
"""
...
def compile_minimal_param_stub():
    """Compile the minimal parameter stub"""
    logging.basicConfig(level=logging.DEBUG)
    
    current_dir = os.path.dirname(os.path.abspath(__file__))
    stub_source = os.path.join(current_dir, "minimal_param_stub.c")
    stub_obj = os.path.join(current_dir, "minimal_param_stub.o")
    stub_elf = os.path.join(current_dir, "minimal_param_stub.elf")
    stub_blob = os.path.join(current_dir, "minimal_param_stub_compiled.bin")
    ...
    try:
        ...
        logging.info(f"Minimal parameter stub compiled successfully to blob: {stub_blob}")
        print(f"Minimal parameter stub blob created at: {stub_blob}")
