#!/usr/bin/env python3
"""
Compilation script for the functional ELF stub
"""
...
def compile_functional_elf_stub():
    """Compile the functional ELF stub"""
    logging.basicConfig(level=logging.DEBUG)
    
    current_dir = os.path.dirname(os.path.abspath(__file__))
    stub_source = os.path.join(current_dir, "functional_stub_elf.c")
    stub_obj = os.path.join(current_dir, "functional_stub_elf.o")
    stub_elf = os.path.join(current_dir, "functional_stub_elf.elf")
    stub_blob = os.path.join(current_dir, "functional_stub_elf_compiled.bin")
    ...
    try:
        ...
        logging.info(f"Functional ELF stub compiled successfully to blob: {stub_blob}")
        print(f"Functional ELF stub blob created at: {stub_blob}")
