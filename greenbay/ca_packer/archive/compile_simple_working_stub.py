#!/usr/bin/env python3
"""
Compilation script for the simple working stub
"""
...
def compile_simple_working_stub():
    """Compile the simple working stub"""
    logging.basicConfig(level=logging.DEBUG)
    
    current_dir = os.path.dirname(os.path.abspath(__file__))
    stub_source = os.path.join(current_dir, "simple_working_stub.c")
    stub_obj = os.path.join(current_dir, "simple_working_stub.o")
    stub_elf = os.path.join(current_dir, "simple_working_stub.elf")
    stub_blob = os.path.join(current_dir, "simple_working_stub_compiled.bin")
    ...
    try:
        ...
        logging.info(f"Simple working stub compiled successfully to blob: {stub_blob}")
        print(f"Simple working stub blob created at: {stub_blob}")
