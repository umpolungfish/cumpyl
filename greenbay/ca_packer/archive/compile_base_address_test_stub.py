#!/usr/bin/env python3
"""
Compilation script for the base address test stub
"""
...
def compile_base_address_test_stub():
    """Compile the base address test stub"""
    logging.basicConfig(level=logging.DEBUG)
    
    current_dir = os.path.dirname(os.path.abspath(__file__))
    stub_source = os.path.join(current_dir, "base_address_test_stub.c")
    stub_obj = os.path.join(current_dir, "base_address_test_stub.o")
    stub_elf = os.path.join(current_dir, "base_address_test_stub.elf")
    stub_blob = os.path.join(current_dir, "base_address_test_stub_compiled.bin")
    ...
    try:
        ...
        logging.info(f"Base address test stub compiled successfully to blob: {stub_blob}")
        print(f"Base address test stub blob created at: {stub_blob}")
...
if __name__ == "__main__":
    compile_base_address_test_stub()