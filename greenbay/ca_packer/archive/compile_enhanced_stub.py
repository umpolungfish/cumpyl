#!/usr/bin/env python3
"""
Compilation script for the enhanced stub
"""
...
def compile_enhanced_stub():
    """Compile the enhanced stub"""
    logging.basicConfig(level=logging.DEBUG)
    
    current_dir = os.path.dirname(os.path.abspath(__file__))
    stub_source = os.path.join(current_dir, "enhanced_stub.c")
    stub_obj = os.path.join(current_dir, "enhanced_stub.o")
    stub_elf = os.path.join(current_dir, "enhanced_stub.elf")
    stub_blob = os.path.join(current_dir, "enhanced_stub_compiled.bin")
    ...
    try:
        ...
        logging.info(f"Enhanced stub compiled successfully to blob: {stub_blob}")
        print(f"Enhanced stub blob created at: {stub_blob}")
