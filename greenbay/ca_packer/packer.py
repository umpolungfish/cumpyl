...
def generate_stub_mvp(oep_rva, key, nonce, ca_params, block_lengths, payload_rva, payload_size, binary_format):
    ...
    if len(stub_data) >= 50:  # Arbitrary threshold to distinguish simple from complex stubs
        ...
        # We'll embed parameters at the end of the stub data
        if len(stub_data) < required_size:
            # Pad the stub data with NOPs (0x90) or zeros
            stub_data.extend(b'\x00' * (required_size - len(stub_data)))
            logging.debug(f"Padded stub data to {len(stub_data)} bytes")
        else:
            # Extend the stub data to make room for parameters
            # We'll add the parameters at the end
            parameter_space = b'\x00' * 0x40  # 64 bytes for parameters
            stub_data.extend(parameter_space)
            STUB_PARAMETER_OFFSET = len(stub_data) - 0x40
            logging.debug(f"Extended stub data to {len(stub_data)} bytes for parameters at offset 0x{STUB_PARAMETER_OFFSET:x}")
        ...
        stub_data[STUB_PARAMETER_OFFSET + 0x38:STUB_PARAMETER_OFFSET + 0x3C] = payload_rva.to_bytes(4, 'little')

        # Payload Size (4 bytes, little-endian)
        stub_data[STUB_PARAMETER_OFFSET + 0x3C:STUB_PARAMETER_OFFSET + 0x40] = payload_size.to_bytes(4, 'little')
    else:
