#!/usr/bin/env python3
"""
Compilation script for the simple minimal exit stub
"""
...
def compile_minimal_exit_stub_simple():
    """Compile the simple minimal exit stub"""
    logging.basicConfig(level=logging.DEBUG)
    
    current_dir = os.path.dirname(os.path.abspath(__file__))
    stub_source = os.path.join(current_dir, "minimal_exit_stub_simple.c")
    stub_obj = os.path.join(current_dir, "minimal_exit_stub_simple.o")
    stub_elf = os.path.join(current_dir, "minimal_exit_stub_simple.elf")
    stub_blob = os.path.join(current_dir, "minimal_exit_stub_simple_compiled.bin")
    ...
    try:
        ...
        logging.info(f"Simple minimal exit stub compiled successfully to blob: {stub_blob}")
        print(f"Simple minimal exit stub blob created at: {stub_blob}")
...
if __name__ == "__main__":
    compile_minimal_exit_stub_simple()