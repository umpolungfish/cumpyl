#!/usr/bin/env python3
"""
Compilation script for the minimal debug stub
"""
...
def compile_minimal_debug_stub():
    """Compile the minimal debug stub"""
    logging.basicConfig(level=logging.DEBUG)
    
    current_dir = os.path.dirname(os.path.abspath(__file__))
    stub_source = os.path.join(current_dir, "minimal_debug_stub.c")
    stub_obj = os.path.join(current_dir, "minimal_debug_stub.o")
    stub_elf = os.path.join(current_dir, "minimal_debug_stub.elf")
    stub_blob = os.path.join(current_dir, "minimal_debug_stub_compiled.bin")
    ...
    try:
        ...
        logging.info(f"Minimal debug stub compiled successfully to blob: {stub_blob}")
        print(f"Minimal debug stub blob created at: {stub_blob}")
