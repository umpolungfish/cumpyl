...
extern long generate_ca_mask_from_key(unsigned char* key_material, unsigned long block_index,
                                       unsigned long ca_steps, unsigned long mask_size,
                                       unsigned char* output_mask);

int main() {
    printf("Testing Full CA Evolution implementation...\n");
    ...
    printf("Calling generate_ca_mask_from_key...\n");
    long result = generate_ca_mask_from_key(key_material, block_index, ca_steps, mask_size, output_mask);
    
    if (result == -1) {
        printf("Full CA mask generation failed!\n");
        return 1;
    } else {
        printf("Full CA mask generation successful!\n");
