#!/usr/bin/env python3
"""
Compilation script for the simple exit stub
"""
...
def compile_simple_exit_stub():
    """Compile the simple exit stub"""
    logging.basicConfig(level=logging.DEBUG)
    
    current_dir = os.path.dirname(os.path.abspath(__file__))
    stub_source = os.path.join(current_dir, "simple_exit_stub.c")
    stub_obj = os.path.join(current_dir, "simple_exit_stub.o")
    stub_elf = os.path.join(current_dir, "simple_exit_stub.elf")
    stub_blob = os.path.join(current_dir, "simple_exit_stub_compiled.bin")
    ...
    try:
        ...
        logging.info(f"Simple exit stub compiled successfully to blob: {stub_blob}")
        print(f"Simple exit stub blob created at: {stub_blob}")
