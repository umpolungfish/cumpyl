#!/usr/bin/env python3
"""
Compilation script for the simple parameter stub
"""
...
def compile_simple_param_stub():
    """Compile the simple parameter stub"""
    logging.basicConfig(level=logging.DEBUG)
    
    current_dir = os.path.dirname(os.path.abspath(__file__))
    stub_source = os.path.join(current_dir, "simple_param_stub.c")
    stub_obj = os.path.join(current_dir, "simple_param_stub.o")
    stub_elf = os.path.join(current_dir, "simple_param_stub.elf")
    stub_blob = os.path.join(current_dir, "simple_param_stub_compiled.bin")
    ...
    try:
        ...
        logging.info(f"Simple parameter stub compiled successfully to blob: {stub_blob}")
        print(f"Simple parameter stub blob created at: {stub_blob}")
