#!/usr/bin/env python3
"""
Compilation script for the minimal working parameter stub
"""
...
def compile_minimal_working_param_stub():
    """Compile the minimal working parameter stub"""
    logging.basicConfig(level=logging.DEBUG)
    
    current_dir = os.path.dirname(os.path.abspath(__file__))
    stub_source = os.path.join(current_dir, "minimal_working_param_stub.c")
    stub_obj = os.path.join(current_dir, "minimal_working_param_stub.o")
    stub_elf = os.path.join(current_dir, "minimal_working_param_stub.elf")
    stub_blob = os.path.join(current_dir, "minimal_working_param_stub_compiled.bin")
    ...
    try:
        ...
        logging.info(f"Minimal working parameter stub compiled successfully to blob: {stub_blob}")
        print(f"Minimal working parameter stub blob created at: {stub_blob}")
...
if __name__ == "__main__":
    compile_minimal_working_param_stub()