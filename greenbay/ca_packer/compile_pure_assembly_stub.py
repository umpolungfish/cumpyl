#!/usr/bin/env python3
"""
Compilation script for the pure assembly stub
"""
...
def compile_pure_assembly_stub():
    """Compile the pure assembly stub"""
    logging.basicConfig(level=logging.DEBUG)
    
    current_dir = os.path.dirname(os.path.abspath(__file__))
    stub_source = os.path.join(current_dir, "pure_assembly_stub.s")
    stub_obj = os.path.join(current_dir, "pure_assembly_stub.o")
    stub_elf = os.path.join(current_dir, "pure_assembly_stub.elf")
    stub_blob = os.path.join(current_dir, "pure_assembly_stub_compiled.bin")
    ...
    try:
        ...
        logging.info(f"Pure assembly stub compiled successfully to blob: {stub_blob}")
        print(f"Pure assembly stub blob created at: {stub_blob}")
