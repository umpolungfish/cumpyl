...
def test_minimal_exit_stub():
    ...
    if not os.path.exists("minimal_exit_v2_packed_binary"):
        print("ERROR: minimal_exit_v2_packed_binary not found")
        return False
        
    # Make sure it's executable
    os.chmod("minimal_exit_v2_packed_binary", 0o755)
    
    # Run the packed binary and check the exit code
    try:
        result = subprocess.run(["./minimal_exit_v2_packed_binary"], 
