#!/usr/bin/env python3
"""
Simple test script to demonstrate CA-Packer usage.
"""
...
def main():
    ...
    packed_program_binary = os.path.join(script_dir, "packed_test_program")
...
""")
    ...
    print(f"Original program output: {result.stdout.strip()}")
    
    # Pack the test program
    print("Packing test program...")
    packer_script = os.path.join(script_dir, "ca_packer", "packer.py")
    result = subprocess.run([
        "python3", packer_script, test_program_binary, packed_program_binary
    ], capture_output=True, text=True)
    ...
    print(f"Successfully packed test program: {packed_program_binary}")
    
    # Make sure the packed program is executable
    os.chmod(packed_program_binary, 0o755)
    
    # Test the packed program
    print("Testing packed program...")
    result = subprocess.run([packed_program_binary], capture_output=True, text=True)
